...
\subsubsection{Use Cases Diagram}
    ...
    \begin{figure}[H]
        ...
        \label{fig:usth_connect_use_case_diagram}
    \end{figure}
...
\subsubsection{Use case: Real Time Notification}
    \begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{image/Real-timeNotification.pdf} 
...
\subsection{K-Mode Algorithm}\cite{huang1998extensions}
% Cost function
...
To determine the optimal number of clusters $K$, the K-Modes clustering model is trained with different values of K varying from 2 to 9 and select the best model according to the cost function. For each training iteration, the K-Modes algorithm is executed multiple times with different initial centroids seeds. The final result is the best output initialized centroids in terms of cost function.\cite{devos2015}
